...
const THRESHOLD_ROWS: { label: string; tooltip: string; enabled: keyof GameMods; pct: keyof GameMods; absolute: keyof GameMods }[] = [
    {
...
function setupThresholdControls(content: Element) {
    ...
    for (const row of THRESHOLD_ROWS) {
        ...
        setupTooltip(button, () => `${row.label}: ${isModEnabled(row.enabled) ? "On" : "Off"}`, () =>
            `${row.tooltip}<br><br>On: skip these Tasks when ${isModEnabled(row.absolute) ? "one rep" : "one skill level"} costs more than the set percentage of max Energy. Off: these Tasks are exempt and always run.`);
        ...
        setupTooltip(mode_button, () => `Metric: % of max Energy ${isModEnabled(row.absolute) ? "per rep" : "per skill level"}`, () =>
            "What the percentage measures for this category.<br><br>/lvl: Energy one rep costs divided by the skill levels it would earn — \"is this worth the Energy as XP?\". Beware on progression-style Tasks: a high skill level makes this explode.<br>/rep: the rep's total Energy cost — \"can I afford to just do this?\".");
