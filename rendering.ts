...
function flashMessage(text: string) {
    ...
    setTimeout(() => { if (div.parentElement === messages) { messages.removeChild(div); } }, 3000);
}
...
function createEditModeBanner(parent: HTMLElement) {
    const banner = createChildElement(parent, "div");
    banner.className = "edit-mode-banner";

    const prev = createChildElement(banner, "button") as HTMLButtonElement;
    ...
    const next = createChildElement(banner, "button") as HTMLButtonElement;
    ...
    const done = createChildElement(banner, "button") as HTMLButtonElement;
...
function createTaskSectionHeader(parent: HTMLElement, title: string): HTMLElement {
...
// artifact-use tasks.
function createArtifactSectionHeader(parent: HTMLElement, rendering: Rendering) {
    const header = createTaskSectionHeader(parent, "Artifacts");

    const add_button = createChildElement(header, "button") as HTMLButtonElement;
    ...
    setupTooltip(add_button, () => "Add Artifact Task", () =>
        "Click here, then click an artifact in your inventory to schedule using it as a task in this zone. Click Add again to cancel.");

    const remove_button = createChildElement(header, "button") as HTMLButtonElement;
    ...
    setupTooltip(remove_button, () => "Remove Artifact Task", () =>
        "Click here, then click a scheduled artifact task to remove it. Click Remove again to cancel.");
}
...
export function populatePrestigeView() {
    ...
    if (automation_allowed && GAMESTATE.prestige_layers_unlocked.length > 0) {
        ...
        const mode_button = createChildElement(queue_controls, "button") as HTMLButtonElement;
        ...
        setupTooltip(mode_button, () => `Queue Purchases: ${prestige_queue_mode ? "On" : "Off"}`, () =>
            "While on, clicking a purchase queues it instead of buying it (unaffordable purchases can be queued too; clicking a repeatable several times queues several levels). Queued purchases are bought automatically, strictly in queue order, whenever you have enough Divine Spark — including Spark earned mid-run and right after a Prestige; the queue survives Prestige. Buttons show their queue position.");

        const reset_button = createChildElement(queue_controls, "button") as HTMLButtonElement;
        reset_button.className = "off";
        reset_button.textContent = `Reset Queue (${GAMESTATE.prestige_buy_queue.length})`;
        (reset_button as HTMLInputElement).disabled = GAMESTATE.prestige_buy_queue.length == 0;
        ...
        const budget_button = createChildElement(queue_controls, "button") as HTMLButtonElement;
...
function setupPersistence(settings_div: HTMLElement) {
    ...
    save_button.addEventListener("click", () => {
        saveGame();
        const save_data = localStorage.getItem(getSaveLocation());
        if (!save_data) {
    ...
    load_button.addEventListener("click", () => {
        ...
        input.addEventListener("change", (e) => {
            ...
            reader.onload = (event) => {
                ...
                localStorage.setItem(getSaveLocation(), fileText as string);
                location.reload();
            };
    ...
    for (const event of events) {
        ...
        switch (event.type) {
            ...
            case EventType.UnlockedPower:
                {
                    ...
                    message_div.innerHTML += `<br>Boosts ${joinWithCommasAndAnd(getPowerSkills().map(getSkillString))}`;
                    recreateTasks();
                    break;
                }
            ...
            case EventType.SkippedZones:
                {
                    message_div.innerHTML = `Skipped to Zone ${GAMESTATE.current_zone + 1} thanks to ${getPerkNameWithEmoji(getZoneSkipCreditPerk())}`;
                    break;
                }
...
function setupInstantModeControl(content: Element) {
    ...
    setupTooltip(button, () => `Instant Mode: ${GAMESTATE.mods.instant_mode ? "On" : "Off"}`, () =>
        "Tasks complete all their remaining reps in a single tick, billing the same Energy and XP normal tick-by-tick play would. Applies to automated and manually-started tasks alike.");
}
...
function setupEditPrioritiesControl(content: Element) {
...
const THRESHOLD_ROWS: { label: string; tooltip: string; enabled: keyof GameMods; pct: keyof GameMods; metric: keyof GameMods; resets: keyof GameMods }[] = [
    {
...
function setupThresholdControls(content: Element) {
    ...
    setupTooltip(master, () => `Energy Thresholds: ${GAMESTATE.mods.threshold_master ? "On" : "Off"}`, () =>
        "Skip prioritized Tasks that fail their category's judgment. Each category picks its metric: /lvl (Energy per skill level earned vs a % of max Energy — \"worth it as XP?\"), /rep (the rep's total Energy vs that % — \"can I afford it?\"), or /rst (estimated Energy Resets until fully completable vs a count — \"reachable soon?\", the default). While Auto Use Items is on and you hold a Scroll of Haste (or one is already queued), every metric judges the Task as if a Scroll will be spent on it (one hasted rep per available Scroll). Each category can be toggled off to exempt it — its Tasks then always run.");
    ...
    for (const row of THRESHOLD_ROWS) {
        ...
        const metric = GAMESTATE.mods[row.metric] as number;
        const skip_when =
            metric == THRESHOLD_METRIC_RESETS ? "completing it would take more Energy Resets than the set count"
        ...
        setupTooltip(button, () => `${row.label}: ${isModEnabled(row.enabled) ? "On" : "Off"}`, () =>
            `${row.tooltip}<br><br>On: skip these Tasks when ${skip_when}. Off: these Tasks are exempt and always run.`);

        const mode_button = createChildElement(row_div, "button") as HTMLButtonElement;
        ...
        setupTooltip(mode_button, () =>
            metric == THRESHOLD_METRIC_RESETS ? "Metric: Energy Resets to complete"
            : `Metric: % of max Energy ${metric == THRESHOLD_METRIC_REP ? "per rep" : "per skill level"}`, () =>
        ...
        if (metric == THRESHOLD_METRIC_RESETS) {
            createNumericInput(row_div, {
                min: 0,
                max: 99,
                initialValue: GAMESTATE.mods[row.resets] as number,
...
function setupAutoFillControl(content: Element) {
    const button = createChildElement(content, "button") as HTMLButtonElement;
...
function setupAutoPrioritizeControl(content: Element) {
    ...
    setupTooltip(button, () => `Auto-Prioritize: ${GAMESTATE.mods.auto_prioritize ? "On" : "Off"}`, () =>
        "Autopilot for priorities: automatically re-runs Auto-Fill Priorities at every Energy Reset and Prestige, when a Task is unlocked, and when you enter a Zone — so the order always reflects your current skills and Energy. Manual priority edits are overwritten while this is on. Mutually exclusive with Queue Cycle.");
}
...
function setupAutoUseCycleControl(content: Element) {
    const on = GAMESTATE.mods.auto_use_cycle;
    const cycle_button = createChildElement(content, "button") as HTMLButtonElement;
    ...
    setupTooltip(cycle_button, () => `Auto Use Cycle: ${GAMESTATE.mods.auto_use_cycle ? "On" : "Off"}`, () =>
        "Cycle Auto Use Items across Energy Resets: keep it off for the set number of resets (banking Items), then on for one reset (spending them), and repeat. Overrides the manual Auto Use Items toggle while enabled. The cycle restarts on Prestige. Mutually exclusive with Queue Cycle.");
...
function setupQueueCycleControl(content: Element) {
    ...
    setupTooltip(button, () => `Queue Cycle: ${GAMESTATE.mods.queue_cycle ? "On" : "Off"}`, () =>
        "Rotate through saved automation queues, advancing one per Energy Reset. Each queue has its own task priorities, scheduled artifact tasks, and whether it's an item cycle (Auto Use Items on). Mutually exclusive with Auto Use Cycle; restarts at the first queue on Prestige.");
    ...
    if (configs.length >= 2) {
        const advance_button = createChildElement(content, "button") as HTMLButtonElement;
        ...
        setupTooltip(advance_button, () => "Advance cycle", () =>
            "Switch to the next queue immediately (wrapping), applying its plan now instead of waiting for the next Energy Reset.");
    }
    ...
    for (let i = 0; i < configs.length; i++) {
        ...
        const name_input = createChildElement(top_row, "input") as HTMLInputElement;
        ...
        const edit_button = createChildElement(row, "button") as HTMLButtonElement;
        ...
        const up_button = createChildElement(row, "button") as HTMLButtonElement;
        up_button.className = "queue-config-btn";
        up_button.textContent = "↑";
        up_button.addEventListener("click", () => { moveQueue(i, -1); setupControls(); });

        const down_button = createChildElement(row, "button") as HTMLButtonElement;
        ...
        const delete_button = createChildElement(row, "button") as HTMLButtonElement;
    ...
    setupTooltip(add_button, () => "Add queue", () =>
        "Save the current task priorities and artifact tasks as a new queue at the end of the cycle.");
}
...
function updateExtraStats() {
    if (GAMESTATE.has_unlocked_power && RENDERING.power_element.classList.contains("hidden")) {
        ...
        setupTooltip(RENDERING.power_element, function () { return `💪Power - ${formatInt(GAMESTATE.power)}`; }, function () {
            let tooltip = `Increases ${joinWithCommasAndAnd(getPowerSkills().map(getSkillString))} speed by ${formatInt(GAMESTATE.power)}%`;
...
export class Rendering {
    ...
    public createTasks() {
        ...
        const exit_tasks: Task[] = [];
        const artifact_tasks: Task[] = [];
        for (const task of GAMESTATE.tasks) {
