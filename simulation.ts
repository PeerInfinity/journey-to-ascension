...
function recordPerformedTaskRep(task: Task) {
    if (isArtifactTaskId(task.task_definition.id)) return;
    const def = task.task_definition;
...
// (ARTIFACTS). Read-only.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
(window as any).getAllItems = () => {
    return (ITEMS as ItemDefinition[])
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
(window as any).grantItem = (item: ItemType | string, count: number = 1) => {
    let type: ItemType | undefined;
    if (typeof item === "number") {
        type = item;
    } else if (typeof item === "string") {
        const match = (ITEMS as ItemDefinition[]).find((def) => def && def.name === item);
        type = match?.enum;
    }
