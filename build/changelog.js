...
export const CHANGELOG = [
    {
        ...
        changes: "- Added Energy Thresholds (Advanced Automation): skip prioritized Tasks that fail a per-category judgment. Eight Task categories (new Perk you can finish this run, new Perk out of reach, pending Task unlock, Combat, awards an Item, Prestige, progression, everything else — perk and unlock categories track what you've already earned/unlocked and demote spent Tasks to later categories, and Bosses always count as Combat rather than Item Tasks), each with its own value, a toggle (a disabled category is exempt and always runs), and a metric switch: /lvl judges Energy per skill level earned vs a % of max Energy, /rep judges the rep's total Energy vs that %, /rst judges how many Energy Resets it would take to fully complete the Task (grinding it with runs like the current one) vs a count. All categories default to /rst; avoid /lvl on progression Tasks — a high skill level makes travelling look worthless per level and can strand the run<br>"
            + "- Energy Threshold judgments account for held Scrolls of Haste: while you have one, a Task is judged as if a Scroll will be spent on it (one hasted rep per held Scroll), so Tasks you'd realistically start with a Scroll aren't skipped for their unhasted cost<br>"
