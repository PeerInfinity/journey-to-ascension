...
export const CHANGELOG = [
    {
        ...
        changes: "- Added Energy Thresholds (Advanced Automation): skip prioritized Tasks that fail a per-category judgment. Six Task categories (new Perk you can finish this run, new Perk out of reach, awards an Item, progression, unlocks a Task, everything else), each with its own value, a toggle (a disabled category is exempt and always runs), and a metric switch: /lvl judges Energy per skill level earned vs a % of max Energy, /rep judges the rep's total Energy vs that %, /rst judges how many Energy Resets it would take to fully complete the Task (grinding it with runs like the current one) vs a count. All categories default to /rst; avoid /lvl on progression Tasks — a high skill level makes travelling look worthless per level and can strand the run<br>"
            + "- When every remaining prioritized Task is over its threshold, choose what happens: Idle (stop and notify), End Run (trigger the Energy Reset), or Best Task (run the skipped Task that would earn the most total skill levels from your remaining Energy, re-chosen as Energy drains)<br>"
