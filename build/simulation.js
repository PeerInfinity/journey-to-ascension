...
let _foreign_award_callback = null;
...
let _perk_category_task_ids = null;
...
export const SKILL_ROLES = {
    ...
    ascension_skill: SkillType.Ascension,
    // The Godly Travel prestige multiplier target.
    travel_skill: SkillType.Travel,
    ...
    attunement_skills: [SkillType.Magic, SkillType.Study],
    power_skills: [SkillType.Combat, SkillType.Fortitude],
    spite_skills: [SkillType.Ascension, SkillType.Charisma],
};
...
function hasTimeCompressionScale() {
...
function hasTimeCompressionSingleTick() {
...
export function getZoneSkipCreditPerk() {
    for (const [perk] of EFFECTS.time_compression_single_tick_run) {
        if (hasPerk(perk)) {
            return perk;
        }
...
let _loaded_dataset_id = null;
let _loaded_dataset_schema_version = null;
export function setLoadedDataset(id, schema_version) {
    _loaded_dataset_id = id;
    _loaded_dataset_schema_version = schema_version;
}
export function getLoadedDatasetId() {
...
function isForcedPerkCategoryTask(def) {
...
function loadGame() {
    ...
    try {
        ...
        const stamp = JSON.parse(saved_game);
        if (_loaded_dataset_id === null) {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
window.getAutomationMode = () => GAMESTATE.automation_mode;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
window.setAutomationMode = (mode) => {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
window.ensureZoneAutomationPriorities = (zone) => {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
window.getPreviousRunActions = () => _previous_run_actions.map(a => ({ ...a }));
// The same log for the CURRENT (in-progress) run, for live inspection.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
window.getCurrentRunActions = () => _current_run_actions.map(a => ({ ...a }));
...
// Pass null to clear.
window.setForeignAwardCallback = (fn) => {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
window.setPerkCategoryTaskIds = (ids) => {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
window.applyTaskPatches = (patches) => {
    const list = [];
    ...
    const resolvePerk = (v) => {
        if (typeof v === 'number')
            return v;
        if (typeof v === 'string')
            return PERKS.find((p) => p.name === v)?.enum;
        return undefined;
    };
    const resolveItem = (v) => {
        if (typeof v === 'number')
            return v;
        if (typeof v === 'string')
            return ITEMS.find((i) => i.name === v)?.enum;
        return undefined;
    };
    const applied = [];
    const skipped = [];
    ...
    for (const { id, fields } of list) {
        ...
        if (typeof fields.cost_multiplier === 'number')
            def.cost_multiplier = fields.cost_multiplier;
        if (typeof fields.xp_mult === 'number')
            def.xp_mult = fields.xp_mult;
        if (typeof fields.max_reps === 'number')
            def.max_reps = fields.max_reps;
        if (typeof fields.hidden_by_default === 'boolean')
            def.hidden_by_default = fields.hidden_by_default;
        if (typeof fields.unlocks_task === 'number')
            def.unlocks_task = fields.unlocks_task;
        if ('perk' in fields) {
            const pk = resolvePerk(fields.perk);
            if (pk !== undefined) {
                def.perk = pk;
                touchedPerkOrItem = true;
            }
        }
        if ('item' in fields) {
            ...
            if (it !== undefined) {
                def.item = it;
                def.item_schedule = undefined;
                touchedPerkOrItem = true;
            }
        }
        applied.push(id);
    }
    if (touchedPerkOrItem)
