...
function setupThresholdControls(content) {
    ...
    const end_run = createChildElement(content, "button");
    ...
    setupTooltip(end_run, () => `End Run When All Skipped: ${isModEnabled("threshold_end_run") ? "On" : "Off"}`, () => "When every remaining prioritized Task is over its Energy Threshold, trigger the Energy Reset instead of idling — leftover Energy was only spendable at rates you've said aren't worth it. Off: automation idles and shows a notification instead.");
