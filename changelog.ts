...
export const CHANGELOG: ChangelogEntry[] = [
    {
        version: "Fork 1.4",
        date: "2026-07-03",
        changes: "- Added Energy Thresholds (Advanced Automation): skip prioritized Tasks that fail a per-category judgment. Six Task categories (new Perk you can finish this run, new Perk out of reach, pending Task unlock, awards an Item, progression, everything else — perk and unlock categories track what you've already earned/unlocked and demote spent Tasks to later categories), each with its own value, a toggle (a disabled category is exempt and always runs), and a metric switch: /lvl judges Energy per skill level earned vs a % of max Energy, /rep judges the rep's total Energy vs that %, /rst judges how many Energy Resets it would take to fully complete the Task (grinding it with runs like the current one) vs a count. All categories default to /rst; avoid /lvl on progression Tasks — a high skill level makes travelling look worthless per level and can strand the run<br>"
        ...
        + "- Added Auto-Fill Priorities (Advanced Automation): one click fills every reached Zone's automation priorities with a sensible order — Item Tasks, then Prestige Tasks (cheap and one-shot, boosted by the fresh Items), Task-unlockers, Perk Tasks (cheapest first), the rest by skill levels per Energy, Mandatory and Travel last<br>"
