...
export const CHANGELOG: ChangelogEntry[] = [
    ...
    },
    {
        ...
        + "- Added a Prestige purchase queue (Divinity popup): toggle Queue Purchases and clicks queue instead of buying (unaffordable purchases included; multiple clicks on a repeatable queue multiple levels). Queued purchases are bought automatically, strictly in queue order, whenever you have enough Divine Spark; the queue survives Prestige. Buttons show their queue positions (entries of one upgrade don't have to be consecutive — queue order is click order); Reset Queue clears it. Auto-Buy Cheapest: while the queue is empty, automatically buy the cheapest affordable purchase<br>"
        + "- Added Auto-Prestige (Advanced Automation): when a run ends and Prestige is available, Prestige instead of resetting if any enabled condition is met — Spark/reset below a percentage of its peak, expected Spark above a target, several resets without a new highest Zone, or expected Spark above a percentage of owned Spark. Honors Resume on Reset<br>"
        + "- Added Show Spark per Reset (Settings, Game Mods): displays, under the Divine Spark button, the Spark a prestige would award averaged over this prestige's runs, plus the peak that average has reached since the last prestige<br>"
        + "- Added Energy Thresholds (Advanced Automation): skip prioritized Tasks that fail a per-category judgment. Eight Task categories (new Perk you can finish this run, new Perk out of reach, pending Task unlock, Combat, awards an Item, Prestige, progression, everything else — perk and unlock categories track what you've already earned/unlocked and demote spent Tasks to later categories, and Bosses always count as Combat rather than Item Tasks), each with its own value, a toggle (a disabled category is exempt and always runs), and a metric switch: /lvl judges Energy per skill level earned vs a % of max Energy, /rep judges the rep's total Energy vs that %, /rst judges how many Energy Resets it would take to fully complete the Task (grinding it with runs like the current one) vs a count. Default judgments (all categories start disabled): Combat and Item /rep 10%, Prestige /rst 10, everything-else /lvl 1%, the rest /rst 3; avoid /lvl on progression Tasks — a high skill level makes travelling look worthless per level and can strand the run<br>"
        + "- Energy Threshold judgments account for Scrolls of Haste: while Auto Use Items is on and you hold one (or one is already queued), a Task is judged as if a Scroll will be spent on it (one hasted rep per available Scroll), so Tasks you'd realistically start with a Scroll aren't skipped for their unhasted cost. The Resets estimate assumes the same Scrolls are available in future resets, including for its grind projection. On banking cycles held Scrolls are ignored, matching Auto Scroll of Haste's gating<br>"
        + "- When every remaining prioritized Task is over its threshold, choose what happens: Idle (stop and notify), End Run (trigger the Energy Reset), or Best Task (run the skipped Task that would earn the most total skill levels from your remaining Energy, re-chosen as Energy drains; ends the run if nothing could convert)<br>"
        ...
        + "- Added Auto-Fill Priorities (Advanced Automation): one click fills every reached Zone's automation priorities with a sensible order — Item Tasks, Combat (Bosses with their unlock done), unearned Perk Tasks (cheapest first), Prestige Tasks (cheap and one-shot, boosted by the fresh Items), Task-unlockers, the rest by skill levels per Energy, Mandatory and Travel last<br>"
        ...
        + "- Added Auto-Fill Order (Advanced Automation): rearrange the category order Auto-Fill and Auto-Prioritize use (Items, Combat, New Perks, Prestige, Unlockers, Everything Else, Mandatory, Travel by default). Changes apply immediately while Auto-Prioritize is on<br>"
    },
    {
        version: "Fork 1.3",
        date: "2026-06-09",
