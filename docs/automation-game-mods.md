...
| **Auto-Fill Priorities** | (Button, not a toggle.) Overwrite every reached zone's automation priorities with a heuristic order: item-awarding tasks, then Prestige tasks (one-shot, cheap for their zone, and the fresh items boost them — e.g. Touch the Divine right after Infuse Mystic Incense), task-unlockers, perk tasks (cheapest to finish first), the rest by skill levels per energy, then Mandatory with Travel last. Re-click after unlocking or reaching new content. |
