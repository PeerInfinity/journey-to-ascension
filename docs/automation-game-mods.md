...
| **Show Spark per Reset** | Under the Divine Spark button, show prospective prestige spark averaged over this prestige's runs (resets so far + the current run), the peak that average has reached since the last prestige (tracked only while prestige is actually available, so it reads as gain / resets-to-first-reach-it rather than run one's on-paper base gain), and how many resets in a row have passed without a new highest zone. Jumps on a new highest zone, decays each reset while plateaued — the raw signals behind the auto-prestige triggers. |
| **Allow Instant Mode** | Settings gate for the Instant Mode toggle in Advanced Automation. Turning it off hides that toggle *and* switches Instant Mode off. |
...
| **Instant Mode** | (Only shown while **Allow Instant Mode** is on in Settings.) Tasks complete all remaining reps in a single tick, billing the same energy and XP normal play would. Works for automated and manual starts. |
