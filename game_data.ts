...
import {
    PrestigeLayer, PrestigeUnlockType, PrestigeRepeatableType,
    PRESTIGE_UNLOCKABLES, PRESTIGE_REPEATABLES,
    type PrestigeUnlock, type PrestigeRepeatable,
...
// MARK: Dataset document types (schema_version 1)
...
interface DsTask {
    ...
    raw_cost?: number;
    raw_xp?: number;
    // Per-rep award schedule (Fork 1.13): exactly max_reps entries, each a
    // local item index or a foreign award {substrate, type, count?}.
    item_schedule?: Array<number | { substrate?: string, type?: string, count?: number }>;
}
...
const ITEM_BEHAVIOR_SLOTS: Record<string, number> = {
...
const PRESTIGE_UNLOCK_BEHAVIOR_KEYS: string[] = [
...
const PRESTIGE_REPEATABLE_BEHAVIOR_KEYS: string[] = [
...
const TASK_TYPE_BY_NAME: Record<string, TaskType> = {
...
const VANILLA_PERK_DEFS: PerkDefinition[] = PERKS.slice();
const VANILLA_ITEM_DEFS: ItemDefinition[] = ITEMS.slice();
const VANILLA_UNLOCK_DEFS: PrestigeUnlock[] = PRESTIGE_UNLOCKABLES.slice();
const VANILLA_REPEATABLE_DEFS: PrestigeRepeatable[] = PRESTIGE_REPEATABLES.slice();

// MARK: Validation (structural essentials)

function isPlaceholder(entry: unknown): boolean {
    return typeof entry === "object" && entry !== null
        && (entry as { placeholder?: unknown }).placeholder === true;
}

export function validateGameDataset(dataset: unknown): string[] {
    const errors: string[] = [];
    ...
    const ds = dataset as JtaDataset;
    ...
    if (!Array.isArray(ds.skills) || skills.length === 0) err("skills must be a non-empty array");
    let live_skills = 0;
    skills.forEach((s, i) => {
        if (isPlaceholder(s)) return;
        live_skills += 1;
        if (typeof s?.name !== "string" || s.name.length === 0) err(`skills[${i}].name must be a non-empty string`);
    ...
    if (live_skills === 0) err("skills must contain at least one non-placeholder entry");
    const validSkill = (idx: unknown): boolean =>
    ...
    const checkRoster = (list: DsRosterEntry[] | undefined, label: string, slots: Record<string, number>): DsRosterEntry[] => {
        const arr = Array.isArray(list) ? list : [];
        if (!Array.isArray(list)) err(`${label} must be an array`);
        const key_by_slot = new Map<number, string>();
        for (const [key, slot] of Object.entries(slots)) key_by_slot.set(slot, key);
        arr.forEach((entry, i) => {
            ...
            for (const e of entry.effects ?? []) {
                if (e?.kind === "skill_speed") {
                    if (!validSkill(e.skill)) err(`${where} skill_speed effect: skill index ${JSON.stringify(e.skill)} is not a live skill`);
                    if (typeof e.add !== "number") err(`${where} skill_speed effect: add must be a number`);
    ...
    const validRosterIndex = (arr: DsRosterEntry[]) => (idx: unknown): boolean =>
        typeof idx === "number" && Number.isInteger(idx) && idx >= 0
    ...
    zones.forEach((zone, zi) => {
        ...
        tasks.forEach((t, ti) => {
            ...
            if (t?.use_item != null && !validItem(t.use_item)) err(`${where}.use_item must be null or a live item index`);
                ...
                if (!Array.isArray(t.item_schedule)) {
                    ...
                    t.item_schedule.forEach((entry, ei) => {
                        if (typeof entry === "number") {
                            if (!validItem(entry)) err(`${where}.item_schedule[${ei}] must be a live item index`);
                            ...
                            if (entry.count != null && !(Number.isInteger(entry.count) && (entry.count as number) > 0)) {
            ...
            if (t?.unlocks_task != null && typeof t.id === "number") unlock_refs.push([t.id, t.unlocks_task]);
        });
    });
    for (const [from, to] of unlock_refs) {
        if (!task_ids.has(to)) err(`task ${from} unlocks_task ${to} does not exist`);
    }
    ...
    if (prestige == null || typeof prestige !== "object") {
        ...
        if (layers.length !== PrestigeLayer.Count) err(`prestige.layers must have exactly ${PrestigeLayer.Count} entries`);
        const checkPrestigeRoster = (
            list: Array<DsPrestigeUnlock | DsPrestigeRepeatable> | undefined,
            label: string, keys: string[], costField: "cost" | "initial_cost",
        ) => {
            const arr = Array.isArray(list) ? list : [];
            ...
            arr.forEach((entry, i) => {
                ...
                const cost = (entry as Record<string, unknown>)[costField];
                if (!(typeof cost === "number" && cost > 0)) err(`prestige.${label}[${i}].${costField} must be a positive number`);
        ...
        if (!Array.isArray(prestige.sbtv_unlock_task_ids)) {
            ...
            for (const id of prestige.sbtv_unlock_task_ids) {
                if (!task_ids.has(id)) err(`prestige.sbtv_unlock_task_ids id ${id} does not exist`);
            }
        }
    }
    ...
    if (roles == null || typeof roles !== "object") {
        ...
        if (!validSkill(roles.ascension_skill)) err("roles.ascension_skill must be a live skill index");
        if (!validSkill(roles.travel_skill)) err("roles.travel_skill must be a live skill index");
        for (const key of ["attunement_skills", "power_skills", "spite_skills"] as const) {
            const arr = roles[key];
...
function setEnumCount(enum_obj: object, new_count: number) {
    const obj = enum_obj as Record<string | number, string | number>;
    const old_count = obj["Count"] as number;
    if (old_count === new_count) return;
    if (obj[old_count] === "Count") delete obj[old_count];
    obj["Count"] = new_count;
    obj[new_count] = "Count";
}

function skillSpeedPairs(effects: DsEffect[] | undefined): [SkillType, number][] {
    const pairs: [SkillType, number][] = [];
    for (const e of effects ?? []) {
        if (e.kind === "skill_speed") pairs.push([e.skill as SkillType, e.add as number]);
    }
    return pairs;
}

function energyOnConsume(effects: DsEffect[] | undefined): number | null {
    for (const e of effects ?? []) {
        if (e.kind === "energy_on_consume") return e.base_amount as number;
    }
    return null;
}

function staticText(text: string | null | undefined): (() => string) | null {
...
function swapSkillTables(ds: JtaDataset) {
    const skills = ds.skills as DsSkill[];
    SKILL_DEFINITIONS.length = 0;
    SKILLS.length = 0;
    skills.forEach((s, i) => {
        const type = i as SkillType;
        if (isPlaceholder(s)) {
...
function swapPerkTables(ds: JtaDataset) {
    const perks = ds.perks as DsRosterEntry[];
    PERKS.length = 0;
    perks.forEach((entry, i) => {
        const slot = i as PerkType;
...
function swapItemTables(ds: JtaDataset) {
    const items = ds.items as DsRosterEntry[];
    ITEMS.length = 0;
    items.forEach((entry, i) => {
        const slot = i as ItemType;
        if (isPlaceholder(entry)) {
        ...
        const def = new ItemDefinition({
            enum: slot,
            name: entry.name as string,
            name_plural: entry.name_plural ?? (entry.name as string),
            icon: entry.icon ?? "",
        ...
        if (entry.behavior != null) {
            ...
            const base = VANILLA_ITEM_DEFS[i] as ItemDefinition;
    ...
    items.forEach((entry, i) => {
        if (!isPlaceholder(entry) && entry.behavior != null && ITEM_BEHAVIOR_SLOTS[entry.behavior] !== undefined) {
            ARTIFACTS.push(i as ItemType);
        }
    });
    NOTE_ITEMS.length = 0;
    for (const idx of ds.item_groups?.["note_items"] ?? []) {
        NOTE_ITEMS.push(idx as ItemType);
    }
}

function swapPrestigeTables(ds: JtaDataset) {
    const prestige = ds.prestige as DsPrestige;
    (prestige.unlockables as DsPrestigeUnlock[]).forEach((entry, i) => {
        const base = VANILLA_UNLOCK_DEFS[i] as PrestigeUnlock;
        if (isPlaceholder(entry)) {
        ...
        PRESTIGE_UNLOCKABLES[i] = {
            type: i as PrestigeUnlockType,
            layer: entry.layer as PrestigeLayer,
            name: entry.name as string,
            get_description: staticText(entry.description) ?? base.get_description,
            cost: entry.cost as number,
        };
    });
    (prestige.repeatables as DsPrestigeRepeatable[]).forEach((entry, i) => {
        const base = VANILLA_REPEATABLE_DEFS[i] as PrestigeRepeatable;
        if (isPlaceholder(entry)) {
            PRESTIGE_REPEATABLES[i] = base;
            return;
        }
        PRESTIGE_REPEATABLES[i] = {
            type: i as PrestigeRepeatableType,
            layer: entry.layer as PrestigeLayer,
            name: entry.name as string,
            get_description: staticText(entry.description) ?? base.get_description,
            initial_cost: entry.initial_cost as number,
            scaling_exponent: entry.scaling_exponent as number,
        };
    });
    PRESTIGE_DATA.spark_zone_origin = prestige.spark_zone_origin as number;
    PRESTIGE_DATA.sbtv_unlock_task_ids = [...(prestige.sbtv_unlock_task_ids as number[])];
}

function swapZoneTables(ds: JtaDataset) {
    const zones = ds.zones as DsZone[];
    ZONES.length = 0;
    zones.forEach((zone, zi) => {
        const tasks = (zone.tasks as DsTask[]).map((t) => new TaskDefinition({
            id: t.id as number,
            name: t.name as string,
            type: TASK_TYPE_BY_NAME[t.type as string] as TaskType,
            cost_multiplier: t.cost_multiplier as number,
            skills: [...(t.skills as number[])] as SkillType[],
            xp_mult: t.xp_mult as number,
            // "None" is null in the dataset; the engine's sentinel is the
            // (dataset-sized) Count value.
            item: (t.item ?? ItemType.Count) as ItemType,
            use_item: (t.use_item ?? ItemType.Count) as ItemType,
            perk: (t.perk ?? PerkType.Count) as PerkType,
            prestige_layer: (t.prestige_layer ?? PrestigeLayer.Count) as PrestigeLayer,
            max_reps: t.max_reps as number,
            hidden_by_default: t.hidden_by_default === true,
            unlocks_task: t.unlocks_task ?? -1,
            zone_id: zi,
            raw_cost: t.raw_cost,
            raw_xp: t.raw_xp,
            item_schedule: t.item_schedule?.map((entry) =>
                typeof entry === "number"
                    ? entry as ItemType
                    : { substrate: entry.substrate as string, type: entry.type as string, count: entry.count ?? 1 }),
        }));
...
function applyRolesAndEconomy(ds: JtaDataset) {
    const roles = ds.roles as DsRoles;
    SKILL_ROLES.ascension_skill = roles.ascension_skill as SkillType;
    SKILL_ROLES.travel_skill = roles.travel_skill as SkillType;
    SKILL_ROLES.attunement_skills = [...(roles.attunement_skills as number[])] as SkillType[];
...
export function applyGameDataset(dataset: unknown): { ok: boolean; alreadyLoaded: boolean; errors?: string[] } {
